...
签到任务统一使用消息事件驱动引擎：收到机器人消息后即时分类处理按钮、验证码图片、计算题、成功/已签到/失败结果。旧导入配置中即使包含 `engine: legacy` 也会被归一化为 `event`，不再提供按动作列表逐步等待和执行的兼容流水线。
...
- `结果命中后不再 OCR` 是否通过，用于确认成功/已签到图片 caption 命中后不会再次识别验证码。
