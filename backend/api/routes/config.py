...
class TelegramConfigResponse(BaseModel):
    ...
    is_custom: bool
    default_api_id: str
    default_api_hash_masked: Optional[str] = None
...
def save_telegram_config(
    ...
    try:
        ...
        api_hash = (request.api_hash or "").strip() or existing.get("api_hash", "")
        if not request.api_id or not api_hash:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
